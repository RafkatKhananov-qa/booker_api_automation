...
class TestUpdateBooking:
    ...
    def test_put_001(self, request_context, api_headers, api_headers_with_cookie):
    ...
    def test_put_002(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        token = r.json()["token"]
    ...
    def test_put_003(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        token = r.json()["token"]

        r = update_booking(request_context, 999999, headers=api_headers_with_cookie(token))
    ...
    def test_put_005(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        ...
        r = update_booking(request_context, booking_id, headers=api_headers_with_cookie("56d620e8836e02b1"))
        assert_status_code(r, 403)

    @allure.story("Негативные сценарии обновления брони")
    @allure.title("Обновление с невалидными данными")
    def test_put_006(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        token = r.json()["token"]
    ...
    def test_put_007(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        token = r.json()["token"]
    ...
    def test_put_008(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        token = r.json()["token"]
    ...
    def test_patch_001(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        token = r.json()["token"]
        ...
        assert_response_fields(r.json(), UPDATED_PRICE_BOOKING_PAYLOAD)

    @allure.story("Успешное обновление брони")
    @allure.title("PATCH: обновление только depositpaid")
    def test_patch_002(self, request_context, api_headers, api_headers_with_cookie):
        r = create_token(request_context, headers=api_headers)
        assert_status_code(r, 200)
        assert_field_in_response_message(r, "token")
        token = r.json()["token"]
        ...
        assert_response_fields(r.json(), UPDATED_DEPOSITPAID_BOOKING_PAYLOAD)
